<script>
    ...
    async function loadRacks() {
        if (selectedWarehouseId === null || selectedWarehouseId === undefined) return;
        try {
            racks = await api.get(`/api/warehouse/racks?warehouse_id=${selectedWarehouseId}`);
        } catch (e) {
    ...
    {:else}
        <div class="main-layout">
            ...
            <div class="sidebar">
                ...
                <!-- Warehouse Selector -->
                <div class="form-group warehouse-selector">
                    <label>Warehouse</label>
                    {#if warehouses.length > 1}
                        <select value={selectedWarehouseId} on:change={handleWarehouseChange}>
                            {#each warehouses as wh}
                                <option value={wh.id}>{wh.name}</option>
                            {/each}
                        </select>
                    {:else if warehouses.length === 1}
                        <div class="single-warehouse">{warehouses[0].name}</div>
                    {:else}
                        <div class="no-warehouse">No warehouses found</div>
                    {/if}
                </div>
                ...
                <div class="rack-form">
                    <div class="form-group">
                        <label>Name</label>
                        <input type="text" bind:value={formData.name} placeholder="e.g. Rack A">
                    </div>
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary" on:click={saveRack}>Save Rack</button>
                ...
                <div class="racks-list">
                    {#each racks as rack (rack.id)}
                        ...
                        >
                            <div class="name">Rack #{rack.id}</div>
                            <div class="meta">{rack.name || `${rack.columns} cols, ${rack.rows} rows`}</div>
                        </div>
                    {/each}
                </div>
            </div>
